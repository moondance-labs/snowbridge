...
contract HelperConfig is Script {
    ...
    constructor(string memory testnet) {
        (activeBeefyClientConfig, activeGatewayConfig, activeGatewayInitConfig) =
            getChainConfig(testnet);
    }

    function getBeefyClientConfig()
        public
        view
        ...
        returns (BeefyClientConfig memory beefyClientConfig)
    {
        beefyClientConfig = BeefyClientConfig({
            randaoCommitDelay: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".randomCommitDelay")), (uint256)
            ),
            randaoCommitExpiration: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".randaoCommitExpiration")), (uint256)
            ),
            minimumSignatures: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".minimumSignatures")), (uint256)
            )
        });
    }
        ...
        returns (GatewayConfig memory gatewayConfig)
    {
        gatewayConfig = GatewayConfig({
            beefyClient: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".beefyClient")), (address)
            ),
            agentExecutor: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".agentExecutor")), (address)
            ),
            bridgeHubParaID: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".bridgeHubParaID")), (ParaID)
            ),
            bridgeHubAgentID: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".bridgeHubAgentID")), (bytes32)
            ),
            foreignTokenDecimals: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".foreignTokenDecimals")), (uint8)
            ),
        ...
        returns (GatewayInitConfig memory gatewayInitConfig)
    {
        gatewayInitConfig = GatewayInitConfig({
            assetHubParaID: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".assetHubParaID")), (ParaID)
            ),
            assetHubAgentID: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".assetHubAgentID")), (bytes32)
            ),
            rejectOutboundMessages: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".rejectOutboundMessages")), (bool)
            ),
            deliveryCost: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".deliveryCost")), (uint128)
            ),
            registerTokenFee: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".registerTokenFee")), (uint128)
            ),
            ...
            assetHubReserveTransferFee: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".assetHubReserveTransferFee")),
                (uint128)
            ),
            exchangeRate: abi.decode(
                vm.parseJson(json, string.concat(jsonPath, ".exchangeRate")), (UD60x18)
            ),
