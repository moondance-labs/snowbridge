...
import {IGatewayV1, IGatewayV2} from "../../src/Types.sol";
import {
    AgentExecuteCommand,
    InboundMessage,
    OperatingMode,
...
import {OperatingMode, ParaID, Command} from "../../src/v1/Types.sol";
import {GatewayProxy} from "../../src/GatewayProxy.sol";
import {MultiAddress} from "../../src/v1/MultiAddress.sol";
...
import {GatewayCoreStorage} from "../../src/storage/GatewayCoreStorage.sol";
...
contract GatewayV1Test is Test {
    ...
    function setUp() public {
        ...
        bridgeHubAgent = IGatewayV1(address(gateway)).agentOf(bridgeHubAgentID);
        assetHubAgent = IGatewayV1(address(gateway)).agentOf(assetHubAgentID);
    ...
    function _makeReportSlashesCommand(uint256 slashFraction)
        public
        pure
        returns (Command, bytes memory)
    {
        IOGateway.Slash[] memory slashes = new IOGateway.Slash[](1);
        slashes[0] = IOGateway.Slash({
            operatorKey: bytes32(uint256(1)), slashFraction: slashFraction, epoch: 1
        });
        uint256 eraIndex = 1;
        return (
            Command.ReportSlashes,
            abi.encode(IOGateway.SlashParams({eraIndex: eraIndex, slashes: slashes}))
        );
    }
    ...
    function _makeReportRewardsCommand() public returns (Command, bytes memory, address) {
        ...
        RegisterForeignTokenParams memory params = RegisterForeignTokenParams({
            foreignTokenID: foreignTokenId, name: "Test", symbol: "TST", decimals: 10
        });

        vm.expectEmit(true, true, false, false);
        emit IGatewayBase.ForeignTokenRegistered(foreignTokenId, address(0));
        ...
        return (
            ...
            abi.encode(
                epoch, eraIndex, totalPointsToken, tokensInflatedToken, rewardsRoot, foreignTokenId
            ),
            tokenAddress
        );
    }
    ...
    function testSendOperatorsData() public {
        ...
        emit IGatewayV1.OutboundMessageAccepted(
            PRIMARY_GOVERNANCE_CHANNEL_ID, 1, messageID, FINAL_VALIDATORS_PAYLOAD
        );
    ...
    function testSendOperatorsDataWith50Entries() public {
        ...
        string memory path =
            string.concat(root, "/test/snowbridge-data/test_vector_message_validator_50.json");
        ...
        vm.expectEmit(true, false, false, true);
        emit IGatewayV1.OutboundMessageAccepted(
            PRIMARY_GOVERNANCE_CHANNEL_ID, 1, messageID, final_payload
        );
    ...
    function testSendOperatorsDataWith400Entries() public {
        string memory root = vm.projectRoot();
        string memory path =
            string.concat(root, "/test/snowbridge-data/test_vector_message_validator_400.json");
        string memory json = vm.readFile(path);
    ...
    function testSendOperatorsDataWith1000Entries() public {
        string memory root = vm.projectRoot();
        string memory path =
            string.concat(root, "/test/snowbridge-data/test_vector_message_validator_1000.json");
        string memory json = vm.readFile(path);
    ...
    function testOwnerCanChangeMiddleware() public {
        vm.expectEmit(true, true, false, false);
        emit GatewayCoreStorage.MiddlewareChanged(
            address(0), 0x0123456789012345678901234567890123456789
        );
    ...
    function testNonOwnerCantChangeMiddleware() public {
        ...
        vm.prank(notOwner);
        vm.expectRevert(abi.encodeWithSelector(IGatewayBase.Unauthorized.selector));
    ...
    function testDecodeSlashes() public {
        ...
        slashes[0] = IOGateway.Slash({operatorKey: alice, slashFraction: 5000, epoch: 500});
        slashes[1] = IOGateway.Slash({operatorKey: bob, slashFraction: 4000, epoch: 400});
        slashes[2] = IOGateway.Slash({operatorKey: charlie, slashFraction: 3000, epoch: 300});

        assertEq(
            abi.encode(IOGateway.SlashParams({eraIndex: eraIndex, slashes: slashes})),
            TEST_VECTOR_SLASH_DATA
        );
    }
    ...
    function testSubmitSlashesWithoutMiddleware() public {
        ...
        emit IOGateway.UnableToProcessSlashMessageB(abi.encodeWithSelector(
                IOGateway.MiddlewareNotSet.selector
            ));
    ...
    function testSubmitSlashesWithMiddlewareComplyingInterfaceAndSlashRevert() public {
        ...
        vm.mockCallRevert(
            address(middleware),
            abi.encodeWithSelector(IMiddlewareBasic.slash.selector),
            "no process slash"
        );

        IOGateway(address(gateway)).setMiddleware(address(middleware));

        IOGateway.Slash memory expectedSlash = IOGateway.Slash({
            operatorKey: bytes32(uint256(1)), slashFraction: SLASH_FRACTION, epoch: 1
        });

        vm.expectEmit(true, true, true, true);
        emit IOGateway.UnableToProcessIndividualSlashB(
            expectedSlash.operatorKey,
            expectedSlash.slashFraction,
            expectedSlash.epoch,
            expectedError
        );
        vm.expectEmit(true, true, true, true);
    ...
    function testSubmitRewardsWithMiddlewareComplyingInterfaceAndRewardsRevert() public {
        ...
        address expectedTokenAddress =
            MockGateway(address(gateway)).tokenAddressOf(expectedForeignTokenId);
        bytes memory expectedBytes = abi.encodeWithSelector(
            IOGateway.EUnableToProcessRewardsB.selector,
            expectedEpoch,
            expectedEraIndex,
